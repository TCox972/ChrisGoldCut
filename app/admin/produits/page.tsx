...
const EMPTY: Draft = { categorie: '', nom: '', description: '', prix: 0, stock: 0 };
...
export default function AdminProduitsPage() {
  ...
  // Rendue comme fonction (pas composant) pour éviter le démontage/remontage
  // à chaque render du parent — ce qui ferait perdre les états locaux internes
  // des enfants (ex: mode "création" de CategorySelect).
  const renderEditableTr = (isNew = false) => (
    <tr key={isNew ? '__new__' : '__edit__'} className={isNew ? 'bg-yellow-50' : 'bg-gray-50'}>
      <td className="px-5 py-3">
        <CategorySelect
          value={draft.categorie}
          onChange={v => hDraft('categorie', v)}
      ...
      <td className="px-5 py-3">
        <div className="flex gap-2">
          <button onClick={isNew ? confirmAdd : saveEdit}
            disabled={saving || !draft.nom.trim() || !draft.categorie.trim()}
            ...
            {loadingP ? (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {produits.map(p => {
                    if (editId === p._id) return renderEditableTr(false);
                    return (
                  ...
                  {adding && renderEditableTr(true)}
                </tbody>
              </table>
            )}
            <div className="px-5 py-4 border-t border-gray-100">
              <button onClick={() => { setEditId(null); setDraft({ ...EMPTY, categorie: availableCategories[0] ?? '' }); setAdding(true); }}
                disabled={adding}
