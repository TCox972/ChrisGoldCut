...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!doc) {
      ...
      // Un employé ne peut pas débloquer un créneau bloqué par l'admin
      if (!isAdmin && doc.adminHeures?.includes(heure)) {
        return NextResponse.json(
          { error: 'Ce créneau a été bloqué par le gérant. Vous ne pouvez pas le débloquer.' },
          { status: 403 },
        );
      }
      doc.heures = doc.heures.filter(h => h !== heure);
      if (isAdmin) {
        doc.adminHeures = (doc.adminHeures || []).filter(h => h !== heure);
      }
